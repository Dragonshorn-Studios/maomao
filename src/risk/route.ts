...
/**
 * Legacy insertion point from the reconciliation PR.
 * Specialist selection now lives in `src/routing` and runs after this
 * classification so resolved/dismissed findings cannot inflate poison-alert.
 * Kept so existing imports and the `currentFindingsForRisk` contract remain stable.
 */
