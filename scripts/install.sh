...
# The GitHub App ID is numeric. Re-prompt (or die when non-interactive) instead
# of writing the app name/slug that fails at the first GitHub API call.
# Usage: prompt_digits VAR REQUIRED(0|1) MSG [DEFAULT]
prompt_digits() {
...
EOF
  ...
  prompt_digits GITHUB_APP_ID 1 "GitHub App ID (numeric)"
  ...
  GITHUB_APP_ID=$(load_env_value "$ROOT/.env" GITHUB_APP_ID)
  [[ "$GITHUB_APP_ID" =~ ^[0-9]+$ ]] \
    || die ".env GITHUB_APP_ID must be the numeric App ID (digits only, e.g. 123456), not the app name or slug"
...
if ! docker compose run -T --rm --no-deps --entrypoint sh maomao -c 'test -r /run/secrets/github-app.pem'; then
