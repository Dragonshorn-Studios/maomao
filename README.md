...
- **Recommended:** set provider keys from the UI at `/config/providers` — Maomao writes them into OpenCode's `auth.json` under the OpenCode data dir (`/opt/opencode/.local/share/opencode/auth.json` on the `maomao-opencode` volume), so they persist across restarts and need no `.env` edits. An env var of the same provider still wins over a stored key; the page shows which source is in effect.
...
- **Opt-in verdicts** (both default to `false`):
  - `GITHUB_REVIEW_ALLOW_APPROVE=true` — publishes `APPROVE` for clean reviews, but only when every specialist finished without errors, the aggregator did not fall back, and the job is not stale. Any degraded run degrades the review to `COMMENT` (reason logged and shown on the job page). A clean review with `POST_EMPTY_REVIEW=false` posts nothing, so no APPROVE can be delivered either. `GITHUB_REVIEW_APPROVE_MAX_SEVERITY` (e.g. `low`) additionally approves reviews whose surviving findings are all at or below that severity — unset keeps the strict clean-only rule. REQUEST_CHANGES keeps precedence: anything at or above `GITHUB_REVIEW_REQUEST_CHANGES_MIN_SEVERITY` never approves.
...
Merged pull requests are terminal review targets: once the merge is recorded, Maomao never starts a review for one. (A review POST already in flight when the merge lands may still complete — see Audit trail.)

- **On merge.** A verified `pull_request.closed` webhook with GitHub's authoritative `merged: true` moves every non-terminal job for that pull request (all head SHAs) to `cancelled` with reason `pr_merged` in a single atomic UPDATE, then drops them from the in-memory queue and stops running pipelines at their next checkpoint. A worker that loses the race to the merge event exits before starting further reviewers and never starts a publish. Merges are recorded per pull, so duplicate or out-of-order deliveries are idempotent: a delayed push arriving after the merge cannot enqueue fresh work (best-effort by construction: the marker holds only merges Maomao received a verified delivery for, keyed by repo full name + PR number, so a rename in between slips past; the manual URL form refuses recorded merged pulls too). Closed-without-merge, uncertain or incomplete payloads, and unauthorized installations cancel nothing.
- **In the UI.** Queued jobs (on the job page and on the `/` cards) show **Dequeue**: one click, atomic, idempotent under repeated clicks; the job records `manual_dequeue`, the operator login (OAuth identity), and a timestamp (it is implemented as a cancellation, so `finished_at` is that timestamp). Running jobs show **Cancel review**, which opens a confirmation page because the review will not be completed; confirming cancels with reason `manual_cancel` and stops the pipeline cooperatively. Under a password gate (no OAuth identity) actions are attributed to "an operator" rather than a fabricated login.
...
- `top of stack <id>: #a, #b, #c` — posted on the **top** PR; validates that every listed PR resolves in the same repository, is declared for the stack at its listed position with a matching count, and chains (`#b` bases on `#a`'s head branch). An invalid trigger gets one actionable error comment; a valid one pins the ordered base/head SHA vector and enqueues exactly one `stack_review` job (deduped per stack id — re-triggers stale a pending run for the same stack, never double-run). The id, colon, and comma separators are all optional (`top of stack #a and #b`); without an id maomao uses the single stack the PR is declared in.
- `start of stack <id>` — posted on the **bottom** PR; records the stack's base (one id per repo; bare `start of stack` generates an id) and leaves a pending marker comment.
- `end of stack <id>` — posted on the **top** PR; resolves the whole stack itself by walking the open-PR base→head branch chain from the trigger PR down to the declared start — no per-member declarations needed. Missing links, ambiguous forks, and cycles each get an actionable error comment. Resolved members are recorded as declarations, so `top of stack` still works as a re-trigger. A bare `end of stack` takes its id from the start marker at the chain's base.

The same markers may live in the **PR body** on a line of their own — optionally wrapped in an HTML comment so they stay invisible (`<!-- start of stack <id> -->`). A body marker suppresses the automatic per-PR review on every handled `pull_request` action until `end of stack` resolves the stack, so stacked PRs skip per-PR checks entirely: `start of stack <id>` on the bottom PR, `part of stack <id>` on every middle PR (suppression only — position comes from the branch layout), `end of stack <id>` on the top PR. `issue X of Y in stack <id>` in a body is also honored as a declaration. Body markers are evaluated on `opened`/`reopened`/`synchronize`/`ready_for_review` events — `edited` is not a subscribed action, so the marker must be in the body before the PR's next event. Authorization matches comment commands (bots need `MAOMAO_STACK_AUTHORS`, humans need collaborator permission); an unverifiable or malformed marker line posts a usage reply and falls through to the normal review — prose can never silently disable it. Unlike comment commands, body-marker runs are automatic work, so a stack review triggered from a body honors the timed repo pause and the per-repo rate limiter.

A comment that clearly intends a stack command but misses the grammar gets a usage reply instead of silence (for authorized actors only).
