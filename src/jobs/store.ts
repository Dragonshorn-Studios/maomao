...
const TERMINAL_SKIP_REQUEUE: JobState[] = ["completed", "publishing", "queued", "preparing", "routing", "reviewing", "aggregating"];
...
export class JobStore {
  ...
  listInterruptedJobs(): JobRow[] {
    return this.db
      .prepare(
        `SELECT * FROM jobs WHERE state IN ('queued', 'preparing', 'routing', 'reviewing', 'aggregating', 'publishing')`,
      )
      .all() as JobRow[];
  }
    ...
    this.db
      .prepare(
        `UPDATE jobs SET state = 'queued', failure_reason = NULL, started_at = NULL, finished_at = NULL,
         ...
         WHERE id = ? AND state IN ('preparing', 'routing', 'reviewing', 'aggregating', 'publishing', 'queued')`,
      )
      .run(updatedAt, id);
    this.db
      .prepare(
